...
- **상태줄**: FIX 여부, 위성 수, HDOP, 저장 점 수, PL(PlayerLoad, IMU 있을 때). fix 전에는 "보이는 위성 N개"로 탐색 진행을 보여준다.
- **상세 패널 (상태줄 탭)**: GPS / IMU 두 탭.
  - GPS: 위도·경도(7자리)·고도·속도·방향·위성·HDOP·UTC, 실측 수신율(Hz), 원시 NMEA 문장(WiFi 경로),
...
- **항법 모드**: 부팅 시 `$PCAS11,2`(보행자)를 보낸다. 공장값인 차량 모드는 저속 정지 필터가 걷는 속도를 0으로 눌러
  거리가 간헐적으로만 쌓이는 증상이 났다(2026-09-15 실외 테스트). 화면의 현재속도는 임계값 적용 전 모듈 원시값이다.
